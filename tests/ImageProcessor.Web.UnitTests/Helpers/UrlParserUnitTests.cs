...
namespace ImageProcessor.Web.UnitTests.Helpers
{
    ...
    public class UrlParserUnitTests
    {
        ...
        {
            Web.Helpers.UrlParser.ParseUrl(url + "?" + query, "", out string requestPath, out string queryString);
        ...
        public void TestRemoteUrlUnencdoded(string baseUrl, string prefix, string path, string query)
        {
            ...
            Web.Helpers.UrlParser.ParseUrl(url, prefix, out string requestPath, out string queryString);
            Assert.True(queryString.Equals(query));
        ...
        public void TestRemoteUrlWithEncodedQuerystring(string baseUrl, string prefix, string path, string query)
        {
            ...
            Web.Helpers.UrlParser.ParseUrl(legacyUrl, prefix, out string requestPath, out string queryString);
            Assert.True(queryString.Equals(query));
            Assert.True(requestPath.Equals(path));
        ...
        [Test]
        [TestCase("http://localhost/ec/123/Deck.jpg?width=100", "ec", "/123/Deck.jpg")]
        public void TestPrefixMatchesPartialUrl(string url, string prefix, string expectedPath)
        {
            Web.Helpers.UrlParser.ParseUrl(url, prefix, out string requestPath, out string queryString);
            Assert.True(requestPath.Equals(expectedPath));
        }
