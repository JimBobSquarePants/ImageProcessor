...
namespace ImageProcessor.Quantizers
{
    ...
    public class WuQuantizer : WuQuantizerBase
    {
        ...
        /// </returns>
        internal override Bitmap GetQuantizedImage(ImageBuffer imageBuffer, int colorCount, Bgra32[] lookups, int alphaThreshold)
        {
        ...
        /// </summary>
        /// <param name="palette">The <see cref="ColorPalette"/> to fill.</param>
        ...
        /// </returns>
        private static IEnumerable<byte[]> IndexedPixels(
            ImageBuffer image,
            Bgra32[] lookups,
            int alphaThreshold,
            PaletteColorHistory[] paletteHistogram)
        {
            ...
            foreach (Bgra32[] pixelLine in image.PixelLines)
            {
                int length = pixelLine.Length;
                for (int i = 0; i < length; i++)
                {
                    Bgra32 pixel = pixelLine[i];
